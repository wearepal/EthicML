...
def test_imagined_examples():
    data: DataTuple = load_data(Toy())
    train_test: Tuple[DataTuple, DataTuple] = train_test_split(data)
    train, test = train_test

    imaginer: PreAlgorithm = Imagine(epochs=100, batch_size=32)
    ...
    save_2d_plot(new_train, './hmmm.png')
    ...
    assert predictions.values[predictions.values == 1].shape[0] == predictions_og.values[predictions_og.values == 1].shape[0]
    assert predictions.values[predictions.values == -1].shape[0] == predictions_og.values[predictions_og.values == -1].shape[0]
