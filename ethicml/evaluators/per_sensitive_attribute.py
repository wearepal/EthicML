        ...
        for s_col in s_columns:
            for unique_s in actual.s[s_col].unique():
                for p_col in pred_column:
                    subset = DataTuple(
                        x=pd.DataFrame(actual.x.loc[actual.s[s_col] == unique_s].get(actual.x.columns),
                                       columns=actual.x.columns).reset_index(drop=True),
                        s=pd.DataFrame(actual.s.loc[actual.s[s_col] == unique_s][s_col],
                                       columns=[s_col]).reset_index(drop=True),
                        y=pd.DataFrame(actual.y.loc[actual.s[s_col] == unique_s][y_col],
                                       columns=[y_col]).reset_index(drop=True))
                    pred_y = pd.DataFrame(predictions.loc[actual.s[s_col] == unique_s][p_col],
